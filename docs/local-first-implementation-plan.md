...
- `scripts/release_readiness_check.mjs` と `npm run release:check` を追加し、実装ファイル、運用引き継ぎ文書、npm script、Windows workflow、mac同梱物、Windowsインストーラー、公開リリース用リポジトリ、GitHub Actions上の配布artifact、Vercel本番env、現地AI結合証跡をまとめて確認できるようにする。通常モードでは外部環境待ちを `pending` として扱い、`--strict` では `pending` もリリース不可として扱う。
...
- `npm run release:check:full` を追加し、ローカルGit状態、公開リリース用リポジトリ、GitHub Actions上の配布artifact、Vercel本番env名、`output/local-ai-integration-result.json` を含む人間向けリリース判定を1コマンドで表示できるようにする。
