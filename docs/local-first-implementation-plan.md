...
- `npm run release:check:full` を追加し、ローカルGit状態、公開リリース用リポジトリ、`RELEASES_GITHUB_TOKEN` Secret名、コード署名用Secret名、自動更新公開用Variable名、GitHub Actions上の配布artifact、Vercel本番env名、Vercel本番スモーク、`output/local-ai-integration-result.json` を含む人間向けリリース判定を1コマンドで表示できるようにする。
