# codex実装指示書 v3: pharmacy-inventory-report
...
## 4.6 タスクE: 報告書作成画面のUI再設計（録音全廃後に着手）

`docs/product-design-audit/` の監査と修正を前提に、次の再設計を行う。対象は `ReportEdit.tsx` / `MedicationListForm.tsx` 中心。

1. **2カラム化（デスクトップ≥1100px）**: 左=入力フォーム（最大幅約760px）、右=固定サマリーレール。レールには①患者ミニカード（氏名/年齢/医療機関/主治医、ケアマネ未登録警告）②定期薬の最短「いつまで分」を大きく表示（最短薬剤名と残日数付き）③保存状態 ④A4ページ数判定と印刷/保存ボタン、を常時表示。狭幅時はレールを上部の折り畳みサマリーバーに変換（既存の960pxカード化と整合させる）
2. **確認系セクションの圧縮**: 訪問・処方情報、基本情報は1行サマリ＋「編集」展開方式にし、書く欄（指導内容）に面積を配分する
3. **計算結果の脱・入力欄化**: 合計残日数と「いつまで分」は枠なしテキスト表示（font-weight 500、`font-variant-numeric: tabular-nums`、右揃え）。数値・日付列はすべて右揃えに統一
4. **フォント**: BIZ UDPゴシック（Google Fonts、ライセンス確認の上アプリに同梱しオフラインでも適用）を画面・帳票PDF共通の書体とする
5. **色の統制**: 青=主要操作と最短いつまで分のみ、琥珀=注意（未保存/ケアマネ未登録/2枚警告）、緑=保存済み。装飾目的の色使用を廃止
6. **定型文チップ**: 対象5項目の各テキストエリア直下に定型文をチップ表示し1クリックで追記/置換。プルダウン廃止
7. **訪問メモの配置**: 指導内容セクションの先頭に訪問メモ＋「メモから下書きを生成」を置き、生成結果は各欄に提案（採用/編集/破棄）として表示
8. 完了条件: 1440px/1100px/960px/390pxの4幅でスクリーンショットを `docs/product-design-audit/` に追加し、保存バー重なり・横はみ出しゼロ。キーボードのみで全入力が完了できる
...
2. タスクB（Supabase削除）
3. タスクC（凍結・確認・小修正）
4. タスクE（UI再設計。タスクA完了後）
5. タスクD（自動更新。本番有効化は署名取得後）
6. 全テスト（`npm run test:local-app`）緑化 → Winインストーラビルド → 実機スモーク
