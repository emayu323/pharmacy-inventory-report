...
export function createReleaseHandoffMarkdown(options = {}) {
    ...
    lines.push(
        ...
        '## 外部操作の明示承認',
        '',
        '- GitHub ActionsのWindowsインストーラーworkflowを実行してよい、と明示された場合だけ `gh workflow run windows-installer.yml` を実行する。',
        '- Vercel Production環境変数を更新してよい、と明示された場合だけ `vercel env rm ... production` と `vercel env add ... production` を実行する。',
        '- Vercelへ入れる導入コード実値、Service Role Key、トークンはこの文書やチャットに書かない。安全な入力経路で設定する。',
        '- 想定コマンド: `gh workflow run windows-installer.yml`',
        '- 想定コマンド: `vercel env add INSTALL_CODE_REGISTRY production`',
        '- 想定コマンド: `vercel env add WINDOWS_INSTALLER_URL production`',
        '',
        '## 注意',
        '',
